...
|---|---|---|---|
| PCL registration score (`getFitnessScore`) | Yes; logged by frozen baseline | Yes | This is a post-registration nearest-neighbor fitness score; lower is better, but it is neither calibrated covariance nor the exact NDT optimized likelihood. Reported PCL registration scores are not interchangeable with reference-pose correctness; sequence/frame score preferences are mixed.
...
| Transformation-probability / PCL registration-score samples | Offline fixed-pose probe only for the cited comparisons; not consistently exposed/recorded by the frozen runtime (Floor R7H says unavailable) | Yes | Do not assume these are currently available to a deployed online policy without a separate instrumentation stage.
