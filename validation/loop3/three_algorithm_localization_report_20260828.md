...
现场录屏已经同时导出为 GIF、MP4 和 HTML 预览页。这里再次直接放回 GIF 版本，便于优先在支持动画的查看器中播放；同时把 MP4 做成可点击附件，避免 Word 阅读器不播放 GIF 时看不到过程；另外保留 HTML 预览页作为备用，并附上局部误匹配和整体路径的两张原始截图，便于和导师沟通具体拐角问题。

![现场录屏 GIF](figures/screen_video_20260828_152939_loop_small.gif)

MP4（点击打开）：`figures/screen_video_20260828_152939.mp4`
...
按连续 3 帧以上、3D 误差大于 0.5 m 判定为明显误匹配段；剔除这些片段后，仅统计其余正常定位部分，结果如下：

| 统计口径 | 剔除段数 | 剔除点数 | 保留点数 | 保留比例 |
| --- | ---: | ---: | ---: | ---: |
| 误差 > 0.5 m 连续片段 | 10 段 | 826 | 3547 | 81.1% |
